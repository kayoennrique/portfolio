  ...
  return (
    <div className={`flex flex-col relative bg-yellow-300/10 shadow-lg shadow-black/[0.2] p-10 rounded-xl sm:mb-28 md:flex-row gap-5 ${!isActive && 'blur-sm'}`}>
      ...
      <div className="flex flex-col justify-between md:h-[251px]">
        <div>
          <p className="text-font-light text-purple-800 font-bold mb-5 text-3xl text-center md:text-left">{name}</p>
          <p className="text-font-light hidden md:block text-slate-600">{description}</p>
        </div>
        <div className="flex justify-start py-5 flex-wrap">
          <div className="flex md:flex">
            {technologies.map((item, index) => (
              <p key={index} className="bg-purple-300 rounded-lg justify-start text-slate-900 py-[2px] px-5 mr-2 mb-2">{item}</p>
            ))}
          </div>
        </div>
        <div className="relative bg-purple-500 rounded-full sm:w-40 mr-auto ml-auto outline-none transition-all focus:scale-110 hover:scale-110 hover:bg-yellow-500 active:scale-105 disabled:scale-100 disabled:bg-opacity-65">
